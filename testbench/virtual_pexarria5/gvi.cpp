...
std::string usage = " -v <verilog-source> -t <top-module> { -c <clk-port> } { -I <verilog-include-path> } { -G <verilator-parameter>=<value> } [-g]\n    -g  extends modulename with a hash of the given generics";


struct Options
{
	template<class T>
	T get_value(int &i, int argc, char *argv[], const std::string &expected) {
		if (i+1 < argc) {
			++i;
	...
	Options(int argc, char *argv[])
		: verilog_source(""), top_module(""), add_generics_hash(false), generics_hash(""), unittest(false), help(false)
	{
		for (int i = 1; i < argc; ++i) {
			...
			else if (argvi == "-G") verilog_parameter_args.push_back(get_value<std::string>(i,argc,argv,"<verilog-parameter-arg>"));
...
bool in_token(const std::string &token) {
	return token.size() >= 5 && token.substr(0,5) == "VL_IN";
}
...
bool out_token(const std::string &token) {
	return token.size() >= 6 && token.substr(0,6) == "VL_OUT";
}
...
std::string cpp_verilator_interface_preface(const std::string &modulename) {
	std::string prefix = function_name_prefix(modulename);
	std::ostringstream out;
...
void write_cpp_file(std::ofstream &cpp_out, const std::vector<Port> &ports, const std::string &modulename)
{
	cpp_out << cpp_verilator_interface_preface(modulename) << std::endl;
	for (auto port: ports) {
		if (port.bitsize <= 64) {
...
void generate_ghdl_verilator_interface(const Options &options) 
{
	...
	write_cpp_file(cpp_out, ports, modulename);
	...
	std::cout << "generate ghdl flags" << std::endl;
