...
std::vector<Vsockit_owm*> sockit_owm_top_instances;
std::vector<VerilatedVcdC*> sockit_owm_tfp_instances;
...
extern "C" {
	int interface_sockit_owm_init(int *pts) {
		int idx = sockit_owm_top_instances.size();
		sockit_owm_top_instances.push_back(new Vsockit_owm);
	    Verilated::traceEverOn(true);   // Verilator must compute traced signals
	    sockit_owm_tfp_instances.push_back(new VerilatedVcdC);
	    sockit_owm_top_instances[idx]->trace(sockit_owm_tfp_instances[idx], 99);   // Trace 99 levels of hierarchy
	    std::ostringstream filename;
	    filename << "vlt_dump_sockit_owm_" << std::setw(2) << std::setfill('0') << std::dec << idx << ".vcd";
    	sockit_owm_tfp_instances[idx]->open(filename.str().c_str()); // Open the dump file
	...
	void interface_sockit_owm_clk(int idx, int clk) {
		sockit_owm_top_instances[idx]->eval();
		// std::cerr << "clk[" << idx << "] = " << clk << std::endl;
		sockit_owm_top_instances[idx]->clk = clk;
	    //if (sockit_owm_tfp_instances[idx]) sockit_owm_tfp_instances[idx]->dump(main_time); // Create waveform trace for this timestamp
	    //main_time += 8;
	}

    // VL_IN8(rst,0,0);
	void interface_sockit_owm_rst(int idx, int rst) {
		sockit_owm_top_instances[idx]->rst = rst;
	}

    // VL_IN8(bus_ren,0,0);
	void interface_sockit_owm_bus_ren(int idx, int bus_ren) {
		sockit_owm_top_instances[idx]->bus_ren = bus_ren;
	}
    // VL_IN8(bus_wen,0,0);
	void interface_sockit_owm_bus_wen(int idx, int bus_wen) {
		...
		// }
		sockit_owm_top_instances[idx]->bus_wen = bus_wen;
	}
    // VL_IN8(bus_adr,0,0);
	void interface_sockit_owm_bus_adr(int idx, int bus_adr) {
		sockit_owm_top_instances[idx]->bus_adr = bus_adr;
	}


    // VL_OUT8(bus_irq,0,0);
	int interface_sockit_owm_bus_irq(int idx) {
		return sockit_owm_top_instances[idx]->bus_irq;
	}
    // VL_OUT8(owr_p,0,0);
	int interface_sockit_owm_owr_p(int idx) {
		return sockit_owm_top_instances[idx]->owr_p;
	}
    // VL_OUT8(owr_e,0,0);
	int interface_sockit_owm_owr_e(int idx) {
		return sockit_owm_top_instances[idx]->owr_e;
	}
    // VL_IN8(owr_i,0,0);
	void interface_sockit_owm_owr_i(int idx, int owr_i) {
		sockit_owm_top_instances[idx]->owr_i = owr_i;
	}
    // VL_IN(bus_wdt,31,0);
	void interface_sockit_owm_bus_wdt(int idx, int bus_wdt) {
		sockit_owm_top_instances[idx]->bus_wdt = bus_wdt-INT_MIN;
	}
    // VL_OUT(bus_rdt,31,0);
	int interface_sockit_owm_bus_rdt(int idx) {
		return sockit_owm_top_instances[idx]->bus_rdt+INT_MIN;
	}
