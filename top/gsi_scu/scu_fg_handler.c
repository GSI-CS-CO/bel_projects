...
ONE_TIME_CALL bool feedAdacFg( FG_REGISTER_T* pThis )
{
    ...
    */
   if( !cbReadSave( &g_shared.oFg.aChannelBuffers[0], &g_shared.oFg.aRegs[0],
                   ...
                   const BUS_BASE_T fgAddrOffset )
{
   ...
   STATIC_ASSERT( sizeof( g_shared.oFg.aRegs[0].ramp_count ) >= 2 * sizeof( pFgRegs->ramp_cnt_low ) );
#endif
   /*
    * Read the hardware ramp counter respectively polynomial counter
    * from the concerning function generator.
    */
   g_shared.oFg.aRegs[channel].ramp_count = MERGE_HIGH_LOW( ADDAC_FG_ACCESS( pFgRegs, ramp_cnt_high ),
