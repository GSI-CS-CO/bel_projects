...
void irq_handler()
{
  ...
  slv_int_act_reg = scub_base[CALC_OFFS(slave_nr) + SLAVE_INT_ACT];
  ...
  if (slv_int_act_reg & 0x2000) { //tmr irq?
    //tmr_irq_cnts = scub_base[CALC_OFFS(slave_nr) + TMR_BASE + TMR_IRQ_CNT];
    // init old_tmr_cnt
...
void configure_timer(unsigned int tmr_value) {
  ...
  while(scub.slaves[i].unique_id) {
    ...
    scub_base[CALC_OFFS(slot) + SLAVE_INT_ENA] = 0x2000;                     // enable tmr irq in slave macro
    scub_base[CALC_OFFS(slot) + TMR_BASE + TMR_CNTRL] = 0x1;                 // reset TMR
    scub_base[CALC_OFFS(slot) + TMR_BASE + TMR_VALUEL] = tmr_value & 0xffff; // enable generation of tmr irqs, 1ms, 0xe848
    scub_base[CALC_OFFS(slot) + TMR_BASE + TMR_VALUEH] = tmr_value >> 16;    // enable generation of tmr irqs, 1ms, 0x001e
    scub_base[CALC_OFFS(slot) + TMR_BASE + TMR_REPEAT] = 0x14;               // number of generated irqs
    i++;
  }
}
...
int configure_fg_macro(int channel) {
  ...
  if (channel >= 0 && channel < MAX_FG_CHANNELS) {
    ...
    if ((slot & 0xf0) == 0) {                                      //scu bus slave
      ...
      if (status = scub_write_mil(scub_base, slot & 0xf, 0x1, 0x60 << 8 | dev) != OKAY) dev_failure(status);   // set FG mode
      if (status = scub_write_mil(scub_base, slot & 0xf, 0x1, FC_CNTRL_WR | dev) != OKAY) dev_failure(status); // reset fg
    }
    ...
    if (cbRead(&fg_buffer[0], &fg_regs[0], channel, &pset)) {
      ...
      if ((slot & 0xf0) == 0) {
        ...
        // transmit in one block transfer over the dev bus
        if(status = scub_write_mil_blk(scub_base, slot & 0xf, &blk_data[0], FC_BLK_WR | dev)  != OKAY) dev_failure(status);
        // still in block mode !
        if(status = scub_write_mil(scub_base, slot & 0xf, cntrl_reg_wr, FC_CNTRL_WR | dev)  != OKAY) dev_failure(status);
    ...
    if ((slot & 0xf0) == 0) {
      ...
      short data;
      if (status = scub_read_mil(scub_base, slot & 0xf, &data, FC_CNTRL_RD | dev) != OKAY) dev_failure(status);
      if (status = scub_write_mil(scub_base, slot & 0xf, 0xffff & data | FG_ENABLED, FC_CNTRL_WR | dev) != OKAY) dev_failure(status); 
    }
...
void reset_slaves() {
  ...
  while(scub.slaves[i].unique_id) {
    disp_put_c('x');
    scub_base[CALC_OFFS(scub.slaves[i].slot) + TMR_BASE + TMR_CNTRL] = 0x1; //reset TMR
    i++;
  }
}
...
void disable_channel(unsigned int channel) {
  ...
  if ((slot & 0xf0) == 0) {
    ...
    // disarm hardware
    if(status = scub_read_mil(scub_base, slot & 0xf, &data, FC_CNTRL_RD | dev) != OKAY)          dev_failure(status); 
    if(status = scub_write_mil(scub_base, slot & 0xf, data & ~(0x2), FC_CNTRL_WR | dev) != OKAY) dev_failure(status); 
    //write_mil(scu_mil_base, 0x0, 0x60 << 8 | dev);             // unset FG mode
  }
...
void sw_irq_handler(unsigned int adr, unsigned int msg) {
  ...
  switch(code) {
    ...
    case 1:
      ...
      scub_base[CALC_OFFS(0xd) + TMR_BASE + TMR_CNTRL] = 0x2; //multicast tmr enable
    break;
    case 2:
      enable_scub_msis(value);
