...
{
   g_shared.oFg.fg_regs[channel].state = STATE_ARMED;
...
{
   const SIGNAL_T signal = cbisEmpty( &g_shared.oFg.fg_regs[0], channel )?
                             IRQ_DAT_STOP_EMPTY : IRQ_DAT_STOP_NOT_EMPTY;

   sendSignal( signal,  channel );
   fgDisableInterrupt( channel );
   g_shared.oFg.fg_regs[channel].state = STATE_STOPPED;
...
{
   g_shared.oFg.fg_regs[channel].state = STATE_ACTIVE;
