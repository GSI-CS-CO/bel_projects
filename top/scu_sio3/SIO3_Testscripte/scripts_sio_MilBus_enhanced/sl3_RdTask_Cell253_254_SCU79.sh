...
echo "write 2 Commands to tx_taskram253 and tx_taskram 254 register "
eb-write $scuname ${tx_taskram_adr[253]}/2 $data1
eb-write $scuname ${tx_taskram_adr[254]}/2 $data2
echo "----------------------------------------------------------------------------"

echo "tx_req_reg 16            =" $(eb-read  $scuname ${tx_req_adr[15]}/2)
echo "rx_status_avail_reg 16   =" $(eb-read  $scuname ${rd_stat_avail_adr[15]}/2)
echo "rx_err_reg 16            =" $(eb-read  $scuname ${rd_err_adr[15]}/2)
echo "----------------------------------------------------------------------------"
echo "read data from taskram253=" $(eb-read $scuname ${rx_taskram_adr[253]}/2) 
echo "----------------------------------------------------------------------------"
echo "tx_req_reg 16            =" $(eb-read  $scuname ${tx_req_adr[15]}/2)
echo "rx_status_avail_reg 16   =" $(eb-read  $scuname ${rd_stat_avail_adr[15]}/2)
echo "rx_err_reg 16            =" $(eb-read  $scuname ${rd_err_adr[15]}/2)
echo "----------------------------------------------------------------------------"
echo "read data from taskram254=" $(eb-read $scuname ${rx_taskram_adr[254]}/2)
echo "----------------------------------------------------------------------------"
echo "tx_req_reg 16            =" $(eb-read  $scuname ${tx_req_adr[15]}/2)
echo "rx_status_avail_reg 16   =" $(eb-read  $scuname ${rd_stat_avail_adr[15]}/2)
echo "rx_err_reg 16            =" $(eb-read  $scuname ${rd_err_adr[15]}/2)
