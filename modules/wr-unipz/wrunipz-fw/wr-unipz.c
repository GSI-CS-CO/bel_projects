...
uint64_t writeTM(uint32_t uniEvt, uint64_t tStart, uint32_t pz, uint32_t virtAcc, uint32_t flagNochop)
{
  uint64_t deadline;
  uint32_t offset;                                  
  uint64_t id;
  ...
  evtData  = evtData | ((flagNochop & 0x1) << 1);    // set 'no chopper' bit
    ...
    if ((evts.validFlags >> i) & 0x1) {                // data is valid?
      if ((evts.evtFlags >> i) & 0x1) {                // data is an event?
        if (((evts.prepFlags >> i) & 0x1) == isPrep) { // data matches 'isPrep condition'
          offset = writeTM(evts.data[i], tStart, pz, virtAcc, nextNochop[pz]);
        } // if 'isPrep'
      } // is event
    } // is valid
  } // for i
...
uint32_t extern_entryActionConfigured()
{
  ...
  if ((status = resetPiggyDevMil(common_getMilPiggy()))  != MIL_STAT_OK) {
  ...
  configLemoOutputEvtMil(common_getMilPiggy(), 2);    // used to see a blinking LED (and optionally connect a scope) for debugging
...
uint32_t extern_entryActionOperation()
{
  ...
  enableFilterEvtMil(common_getMilPiggy());                  // enable MIL event filter
  clearFifoEvtMil(common_getMilPiggy());                     // clear MIL event FIFO
