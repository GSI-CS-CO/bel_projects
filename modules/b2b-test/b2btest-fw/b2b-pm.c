...
uint32_t ioCtrlSetGate(uint32_t enable, uint32_t io)  // set gate of LVDS input
{
  uint32_t offset;
  
  if ((enable != 1) && (enable != 0)) return B2BTEST_STATUS_OUTOFRANGE;
  if (io > 31)                        return B2BTEST_STATUS_OUTOFRANGE;
...
uint32_t doActionS0()
{
  ...
  if (findIOCtrl()   != B2BTEST_STATUS_OK) status = B2BTEST_STATUS_ERROR;
                           ...
                           uint32_t actStatus)                // actual status of firmware
{
  ...
  case B2BTEST_ECADO_B2B_PMEXT :
    ...
    nInput = 0;
    ioCtrlSetGate(1, 2);                                      // enable input gate
    ...
    ioCtrlSetGate(0, 2);                                      // disable input gate 
