...
signal vcnt		: std_logic_vector(4 downto 0);	-- Vorteiler um aus den 300MHz herunter zu kommen
...
begin
	if (clrn = '0') then
		...
		WHEN wrpause1 =>		if (vcnt(4) = '1') then
									auftrag <= wrpause2;
								end if;
								
		WHEN wrpause2 =>		if (vcnt(4) = '1') then
									auftrag <= to_rdstat;
								end if;
...
begin
	if ( clrn = '0') then
		...
		enas	<= vcnt(4);
		if (vcnt(4) = '1') then
			vcnt	<= conv_std_logic_vector(15	,5);-- vorläufig
		else
			vcnt	<= vcnt - 1;
		end if;
	end if;
