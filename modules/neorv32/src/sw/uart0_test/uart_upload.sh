...
FILESIZE=$(stat -c%s "$2")
TIMEOUT=$(( (FILESIZE / (BAUD/10)) + 10 ))

# setup serial port
printf "Opening serial port ($1)... "
stty -F $1 $BAUD -hup raw -echo cs8 -cstopb -ixon clocal cread    #set tty as 8N1: 8bit, no, 1 stop bit
exec 3<>$1     # build symbol 3, the file refers to $1(serial port), <> Readable/Writeable, after this can use &3 to read/write $1
printf "OK\n"
...
while [ $SECONDS -lt $EXPIRED ]; do
  if read -r -n 1 -u 3 -t 1 CH; then   #read char from terminal &3
    BUFFER+=$CH
    UPOLOAD_OK=-1
