...
volatile ECA_QUEUE_ITEM_T* pECAQ;   // WB address of ECA queue
...
void ecaHandler( void )
{
  // read flag and check if there was an action
   ...
   switch( pECAQ->tag )
   {
      case MY_ECA_TAG:
      {
          mprintf( "EvtID: 0x%08x%08x; deadline: 0x%08x%08x; flags: 0x%08x\n",
                   pECAQ->eventIdH,
                   pECAQ->eventIdL,
                   pECAQ->deadlineH,
                   pECAQ->deadlineL,
...
void main(void) {
  ...
  mprintf("***");
  for (j = 0; j < (31000000); ++j) { asm volatile ("nop"); }     // wait 1 second
  ...
  while (1) {
    ...
    *pSharedCounter = i;
    ...
    for (j = 0; j < (3200); ++j) { asm volatile ("nop"); }
  } // while
} /* main */
