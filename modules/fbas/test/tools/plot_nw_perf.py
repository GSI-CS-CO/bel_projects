...
pattern = re.compile(r"nw_perf_(\d+)\.csv")
...
if not datasets:
    raise FileNotFoundError("No nw_perf_<i>.csv files found")
...
plt.title("C2 Messaging Latency (~6K messages by SCUs)")
plt.grid(True, linestyle="--", alpha=0.5)

# legend proxy (avoid clutter)
for k, (idx, _) in enumerate(datasets):
    plt.scatter([], [], color=cmap(k % 10), label=f"{idx} SCU(s)")
