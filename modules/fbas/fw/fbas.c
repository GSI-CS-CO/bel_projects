...
static uint32_t handleEcaEvent(uint32_t pollTimeout, uint32_t* mpsTask, msgCtrl_t* pMsgCtrl, mpsMsg_t** head)
{
  ...
  if (nextAction != COMMON_ECADO_TIMEOUT) {
    ...
    uint64_t nodeId;  // node ID (MAC address), higher 6 bytes in the 'param' field
    ...
    switch (nextAction) {
      ...
      case FBAS_WR_FLG:
        if (nodeType == FBAS_NODE_RX) { // FBAS RX re-generates MPS class 2 signals
          ...
          {
            *(pSharedApp + (FBAS_SHARED_BAD_MSG_CNT >> 2)) = measureCountEvt(BAD_MSG_CNT, 1);
          }
      ...
      case FBAS_NODE_REG:
        ...
        nodeId = ecaParam >> 16;                  // node ID (MAC address)
        ...
        if (nodeType == FBAS_NODE_RX) {  // registration request from TX
          if (flag == REG_FLAG) {
            // find the given sender node ID in the dedicated array and return its channel ID
