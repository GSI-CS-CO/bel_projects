...
void buildHeader()
{
  sprintf(headerK, "|        pattern name | t_last [UTC] | orign | sid| kick set    trg offst start fltop | destn | sid| phase set get  | kick  set    trg offst start fltop Doffst  TOF |");
  sprintf(emptyK,  "|                     |              |       |    |                                   |       |    |                |                                                |");
...
void buildPrintLine(uint32_t idx)
{
  ...
  switch (set_mode[idx]) {
    ...
    case B2B_MODE_B2BFBEAT : sprintf(dest, "%s", tmp1); flagTCBS = 1; flagOther = 1; flagB2b = 1; flagCphase = 0; flagExtTrig = 1; flagInjTrig = 1; break;
    ...
    default:               sprintf(dest, TXTUNKWN);     flagTCBS = 0; flagOther = 0; flagB2b = 0; flagCphase = 1; flagExtTrig = 0; flagInjTrig = 0; break;
  } // switch set_mode
