...
echo -e B2B start script for SIS100 rf room INT
...
# lm32 listens to CMD_B2B_PMEXT message from SIS100 CBU
# no extraction to another ring
saft-ecpu-ctl $SDPM -c 0x13b0800000000000 0xfffffff000000000 0 0x800 -d
#saft-ecpu-ctl $SDPM -c 0x13a1800000000000 0xfffffff000000000 0 0x800 -d

# lm32 listens to CMD_B2B_TRIGGEREXT message from SIS100 CBU - match diagnostic
saft-ecpu-ctl $SDPM -c 0x1136804000000000 0xfffffff000000000 20000 0x804 -dg

# lm32 listens to >>delayed<< (CMD_B2B_PMEXT) message from SIS100 CBU: B2B_ECADO_B2B_PDEXT - phase diagnostic
# no extraction to another ring
saft-ecpu-ctl $SDPM -c 0x13b0800000000000 0xfffffff000000000 15900000 0x820 -d
#saft-ecpu-ctl $SDPM -c 0x13a1800000000000 0xfffffff000000000 15900000 0x820 -d

# diag: generate pulse upon CMD_B2B_START message from SIS100 CBU
saft-io-ctl $SDPM -n IO2 -o 1 -t 0
saft-io-ctl $SDPM -n IO2 -c 0x113681f000000000 0xfffffff000000000 500000 0x0 1 -u
saft-io-ctl $SDPM -n IO2 -c 0x113681f000000000 0xfffffff000000000 10000000 0x0 0 -u
...
saft-ecpu-ctl $SDCBU -c 0x113681f000000000 0xfffffff000000000 0 0x81f -d

# lm32 listens to CMD_B2B_PREXT message from extraction machine, 200us pretrigger
# no extraction to another ring
saft-ecpu-ctl $SDCBU -c 0x13b0802000000000 0xfffffff000000000 200000 0x802 -dg
...
#saft-io-ctl $SDCBU -n IO1 -c 0x113681f000000000 0xfffffff000000000 0 0x0 1 -u
#saft-io-ctl $SDCBU -n IO1 -c 0x113681f000000000 0xfffffff000000000 10000000 0x0 0 -u
