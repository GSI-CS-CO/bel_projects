...
#define WRMIL_ECADO_BLA              0xa01   // command: bla
...
#define WRMIL_SHARED_SET_UTC_DELAY         (WRMIL_SHARED_SET_UTC_TRIGGER      + _32b_SIZE_)  // delay [us] between the 5 generated UTC MIL events
#define WRMIL_SHARED_SET_TRIG_UTC_DELAY    (WRMIL_SHARED_SET_UTC_DELAY        + _32b_SIZE_)  // delay [us] between the trigger event and the first UTC (and other) generated events
#define WRMIL_SHARED_SET_GID               (WRMIL_SHARED_SET_TRIG_UTC_DELAY   + _32b_SIZE_)  // for internal use: register to hold the source configuration: 1 = SIS ; 2 = ESR ; 0 not configured
...
#define WRMIL_SHARED_SET_UTC_OFFSET_HI     (WRMIL_SHARED_SET_LATENCY          + _32b_SIZE_)  // delay [ms] between the TAI and the MIL-UTC, high word 
#define WRMIL_SHARED_SET_UTC_OFFSET_LO     (WRMIL_SHARED_SET_UTC_OFFSET_HI    + _32b_SIZE_)  // delay [ms] between the trigger the MIL-UTC, low  word
...
#define WRMIL_SHARED_SET_MIL_DEV           (WRMIL_SHARED_SET_REQUEST_FILL_EVT + _32b_SIZE_)  // wishbone address of MIL device; MIL device could be a MIL piggy or a SIO
...
#define WRMIL_SHARED_END                   (WRMIL_SHARED_GET_MSI_SLOT         + _32b_SIZE_) 
