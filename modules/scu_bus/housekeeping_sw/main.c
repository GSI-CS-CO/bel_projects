...
int main(void)
{
  ...
  pp_printf("aru_base: 0x%x\n", aru_base);
  pp_printf("scu_reg: 0x%x\n", scu_reg);
  pp_printf("asmi_base: 0x%x\n", asmi_base);
  pp_printf("BASE_UART: 0x%x\n", BASE_UART);
  pp_printf("BASE_ONEWIRE: 0x%x\n", BASE_ONEWIRE);

  if ((int)aru_base == ERROR_NOT_FOUND) {
    pp_printf("no remote update controller found!\n");
    while (1) {};
  }
  if ((int)scu_reg == ERROR_NOT_FOUND) {
    pp_printf("no WB to SCU memory found!\n");
    while (1) {};
  }
  if ((int)asmi_base == ERROR_NOT_FOUND) {
    pp_printf("no ASMI controller found!\n");
    while (1) {};
  }
  if ((int)BASE_ONEWIRE == ERROR_NOT_FOUND) {
    pp_printf("no 1Wire controller found!\n");
    while (1) {};
  }
