...
boost::optional<const std::string&> HashMap::lookup(const uint32_t hash)     const  { try { return hm.left.at(hash);}  catch (...) {throw std::runtime_error("Hash not found"); return boost::optional<const std::string&>(); }  }

boost::optional<const uint32_t&>    HashMap::lookup(const std::string& name) const  { try { return hm.right.at(name);} catch (...) {throw std::runtime_error("Name not found"); return boost::optional<const uint32_t&>();    }  }  
