...
class SimultaneousThreads(dm_testbench.DmTestbench):
  ...
  def generateSchedule(self, fileName, threadQuantity):
    ...
    lines.append(f'  StartThread [type=startthread shape=triangle color=cyan pattern=MAIN startoffs=0 thread={int(threadMask, 0)}]')
    ...
    for i in range(1,threadQuantity):
      lines.append(f'  Evt_{i:02d} [pattern={i:02d} toffs={i * 300000} par={i:02d}]')
