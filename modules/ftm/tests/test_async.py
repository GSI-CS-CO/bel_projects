...
class UnitTestAsync(dm_testbench.DmTestbench):
  ...
  def test_dynamic_async(self):
    ...
    self.startAndCheckSubprocess((self.binaryDmCmd, self.datamaster, '-i', self.schedules_folder + 'dynamic-async-command-0a.dot'))
    ...
    self.startAndCheckSubprocess((self.binaryDmCmd, self.datamaster, '-i', self.schedules_folder + 'dynamic-async-command-0b.dot'))
    ...
    self.startAndCheckSubprocess((self.binaryDmCmd, self.datamaster, '-i', self.schedules_folder + 'dynamic-async-command-0c.dot'))
    stdoutLines = self.startAndGetSubprocessStdout([self.binaryDmCmd, self.datamaster, 'rawqueue', 'BLOCK_B'])
    self.compareExpectedOutput(stdoutLines, self.schedules_folder + 'dynamic-async-expected-1-5.txt', excludeField='VTIME')
    self.startAndCheckSubprocess((self.binaryDmCmd, self.datamaster, '-i', self.schedules_folder + 'dynamic-async-command-0d.dot'))
    self.delay(1.2)
    stdoutLines = self.startAndGetSubprocessStdout([self.binaryDmSched, self.datamaster, 'rawvisited'])
