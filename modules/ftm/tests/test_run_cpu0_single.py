...
class UnitTestRunCpu0Single(dm_testbench.DmTestbench):
  ...
  def test_dynamic_run_cpu0_single(self):
    ...
    self.compareExpectedOutput(stdoutLines, self.schedules_folder + 'dynamic-basic-run_cpu0_single-expected-0-0.txt')
    self.startAndCheckSubprocess((self.binary_dm_cmd, self.datamaster, 'startpattern', 'IN0'))
    self.delay(0.1)
    stdoutLines = self.startAndGetSubprocessStdout((self.binary_dm_sched, self.datamaster, 'rawvisited'))
    self.compareExpectedOutput(stdoutLines, self.schedules_folder + 'dynamic-basic-run_cpu0_single-expected-1-1.txt')
    self.startAndCheckSubprocess((self.binary_dm_cmd, self.datamaster, 'startpattern', 'IN1'))
    self.delay(0.1)
    stdoutLines = self.startAndGetSubprocessStdout((self.binary_dm_sched, self.datamaster, 'rawvisited'))
    self.compareExpectedOutput(stdoutLines, self.schedules_folder + 'dynamic-basic-run_cpu0_single-expected-2-1.txt')
