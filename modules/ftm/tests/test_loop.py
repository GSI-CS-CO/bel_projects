...
class UnitTestLoop(dm_testbench.DmTestbench):
  ...
  def test_dynamic_loop(self):
    ...
    if self.threadQuantity == 8:
      fileName0 = self.schedules_folder + 'dynamic-loop-expected-0-1.txt'
      fileName1 = self.schedules_folder + 'dynamic-loop-expected-1-1.txt'
    else:
      fileName0 = self.schedules_folder + 'dynamic-loop-expected-0-1-thread32.txt'
      fileName1 = self.schedules_folder + 'dynamic-loop-expected-1-1-thread32.txt'
