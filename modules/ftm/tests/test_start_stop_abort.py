...
class UnitTestStartStopAbort(dm_testbench.DmTestbench):
  ...
  def test_dynamic_start_abort(self):
    ...
    self.startAndCheckSubprocess((self.binary_dm_cmd, self.datamaster, 'abortpattern', 'IN_C0'))
    stdoutLines = self.startAndGetSubprocessStdout((self.binary_dm_cmd, self.datamaster, 'rawstatus'))
  ...
  def test_dynamic_start_stop(self):
    ...
    stdoutLines = self.startAndGetSubprocessStdout((self.binary_dm_cmd, self.datamaster, 'rawstatus'))
    self.compareExpectedOutput(stdoutLines, self.schedules_folder + 'dynamic-basic-start_stop_abort-expected-0-3.txt')
    self.startAndCheckSubprocess((self.binary_dm_cmd, self.datamaster, 'stoppattern', 'IN_C0'))
    stdoutLines = self.startAndGetSubprocessStdout((self.binary_dm_cmd, self.datamaster, 'rawstatus'))
