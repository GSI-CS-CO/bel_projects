...
class NoopTests(dm_testbench.DmTestbench):
  ...
  @pytest.mark.thread8
  def testInspectNoop(self):
    self.runInspectNoopThreads()

  @pytest.mark.thread32
  def testInspectNoop32(self):
    self.threadQuantity = 32
    self.runInspectNoopThreads()

  def runInspectNoopThreads(self):
    """Load a schedule and start all threads. Check that these are running.
    Inspect the noop of some threads. Check that these are not running.
    """
