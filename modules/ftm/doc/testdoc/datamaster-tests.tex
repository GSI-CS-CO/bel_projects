...
\texttt{test\_dm\_cmd\_clearCPUdiag.py} tests \texttt{dm-cmd $<$datamaster$>$ clearCPUdiag} for different CPUs and threads.
Run \texttt{clearCPUdiag} for CPU 0 and 1 the threads 2, 4, 6, 8. Check the result with
\texttt{dm-cmd $<$datamaster$>$ diag}.
...
Run \texttt{cursor} for CPU 0 and 1 the threads 2, 4, 6, 8. Check for the correct number
of output lines.

\section{test\_dm\_cmd\_deadline\_preptime\_startime.py}
\texttt{test\_dm\_cmd\_deadline\_preptime\_startime.py.py} tests \texttt{dm-cmd $<$datamaster$>$ deadline},
\texttt{dm-cmd $<$datamaster$>$ prepline}, and \texttt{dm-cmd $<$datamaster$>$ startline}
for different CPUs and threads.
Tests the commands \texttt{origin, cursor, starttime, preptime, deadline} for different combinations  of threads.
With \texttt{preptime}, all combinations for the threadbits between 0 and 255 ist tested. This is every subset of a
set of 8 threads. In addition, invalid thread masks and thread numbers out of range are tested.

\section{test\_dm\_cmd\_force.py}
\texttt{test\_dm\_cmd\_force.py} tests \texttt{dm-cmd $<$datamaster$>$ force} for different CPUs and threads.
Run \texttt{force} for CPU 0 and 1 the threads 2, 4, 6, 8. Check for the correct number
of output lines.
...
\item testInspectHeap run \texttt{heap} for CPU 0 and 1 the threads 2, 4, 6, 8. Check for the correct number
of output lines. Run \texttt{heap} for each thread on each CPU.
\end{enumerate}
...
Run \texttt{origin} for CPU 0 and 1 the threads 2, 4, 6, 8. Check for the correct number
of output lines.
