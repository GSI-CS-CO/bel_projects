...
namespace coverage {
  ...
  bool CarpeDMInterface::CarpeDM::tableCheck(std::string& report) {
    Graph& g        = gDown;
  ...
  std::string& CarpeDMInterface::CarpeDM::inspectQueues(const std::string& blockName, std::string& report) {
...
std::string& CarpeDMInterface::CarpeDM::getRawQReport(const std::string& blockName, std::string& report) {
  ...
  QueueReport& CarpeDMInterface::CarpeDM::getQReport(const std::string& blockName, QueueReport& qr) {
    Graph& g = gDown;
    AllocTable& at = atDown;
    vStrC futureOrphan;
  ...
  QueueReport& CarpeDMInterface::CarpeDM::getQReport(Graph& g, AllocTable& at, const std::string& blockName, QueueReport& qr, const vStrC& futureOrphan) {
  ...
  QueueElement& CarpeDMInterface::CarpeDM::getQelement(Graph& g, AllocTable& at, uint8_t idx, amI allocIt, QueueElement& qe, const vStrC& futureOrphan) {
...
void CarpeDMInterface::CarpeDM::dumpNode(const std::string& name) {
...
bool CarpeDMInterface::CarpeDM::isPainted(const std::string& name) {
  Graph& g = gDown;
  if (hm.contains(name)) {
    auto it = atDown.lookupHash(hm.lookup(name));
    auto* x = (AllocMeta*)&(*it);
...
void CarpeDMInterface::CarpeDM::showPaint() {
  Graph& g = gDown;
  BOOST_FOREACH( vertex_t v, vertices(g) ) {
...
void CarpeDMInterface::CarpeDM::inspectHeap(uint8_t cpuIdx) {
  vAdr vRa;
  vBuf heap;
...
vEbwrs& CarpeDMInterface::CarpeDM::clearHealth(vEbwrs& ew) {
  for(int cpuIdx = 0; cpuIdx < ebd.getCpuQty(); cpuIdx++) { clearHealth(ew, cpuIdx); }
  return ew;
}


vEbwrs& CarpeDMInterface::CarpeDM::clearHealth(vEbwrs& ew, uint8_t cpuIdx) {
...
HealthReport& CarpeDMInterface::CarpeDM::getHealth(uint8_t cpuIdx, HealthReport &hr) {
  uint32_t const baseAdr = atDown.getMemories()[cpuIdx].extBaseAdr + atDown.getMemories()[cpuIdx].sharedOffs;
...
void CarpeDMInterface::CarpeDM::show(const std::string& title, const std::string& logDictFile, TransferDir dir, bool filterMeta ) {
...
std::vector<std::vector<uint64_t>> CarpeDMInterface::CarpeDM::coverage3TestData(uint64_t seedStart, uint64_t cases, uint8_t parts, uint8_t percentage ) {
...
void CarpeDMInterface::CarpeDM::coverage3Upload(uint64_t seed ) {
...
bool CarpeDMInterface::CarpeDM::coverage3IsSeedValid(uint64_t seed) {
...
std::string CarpeDMInterface::CarpeDM::coverage3GenerateCursor(Graph& g, uint64_t seed ) {
...
Graph& CarpeDMInterface::CarpeDM::coverage3GenerateBase(Graph& g) {
  patEntry.clear();
  patExit.clear();
...
Graph& CarpeDMInterface::CarpeDM::coverage3GenerateStatic(Graph& g, uint64_t seed ) {
...
Graph& CarpeDMInterface::CarpeDM::coverage3GenerateDynamic(Graph& g, uint64_t seed) {
  //cursor position 3 bit (4b)
