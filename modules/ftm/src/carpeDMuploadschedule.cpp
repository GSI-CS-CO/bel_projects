    ...
    BOOST_FOREACH( vertex_t v, vertices(gUp) ) {
      ...
      if(gUp[v].np == nullptr) {
        ...
        completeId(v, gUp); // create ID from SubId fields or vice versa
             ...
             if (cmp == dnt::sTMsg)        {gUp[v].np = (node_ptr) new  TimingMsg(gUp[v].name, x->hash, x->cpu, x->b, 0, s2u<uint64_t>(gUp[v].tOffs), s2u<uint64_t>(gUp[v].id), s2u<uint64_t>(gUp[v].par), s2u<uint32_t>(gUp[v].tef), s2u<uint32_t>(gUp[v].res)); }
