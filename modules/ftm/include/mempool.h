...
public:  
  ...
  const vBuf getBmp()           { return bmp; }
  
  uint32_t getFreeChunkQty()  { return pool.size(); }
  uint32_t getFreeSpace()     { return pool.size() * _MEM_BLOCK_SIZE; }
  uint32_t getUsedSpace()     { return nodeQty - (pool.size() * _MEM_BLOCK_SIZE); }
