...
unsigned int *pMILPiggy;      /* WB address of MIL device bus (MIL piggy)                                   */
...
volatile uint32_t *pSharedData ; /* pointer to a "user defined" n x u32 register; here: data                */
uint32_t *pCpuRamExternal;    /* external address (seen from host bridge) of this CPU's RAM                 */
uint32_t *pSharedDataExternal;  /* external address (seen from host bridge) of data                         */
...
void findRemotePPSGen(){
  int      i;

  // strange name ... this doesn't do what it says on the box! I'd expect some configuration going on, not pure pointer assignment
...
void checkEtherboneSync(uint32_t *inSync, uint32_t *dT)
{
...
void initSharedMem()
{
  ...
  /* Er... Why the individual pointers when you are obviously using an array?
  / this is just confusing, you could easily do this as 
  / pSharedData = (uint32_t *)(pShared + (DMUNIPZ_SHARED_DATA >> 2))
  /
  / Value ...
  /
  / pSharedData[idx] = <value>
  /
  / or Ptr use ...
  /
  / for (idx=0; idx < SHARED_DATA_FIELDS; idx++) {
  / mprintf(" ... 0x%08x\n", (uint32_t)&pSharedData[idx]  ...
  /
  / etc pp
  */
...
// errr ... is it ? you could scan for the name, but I think this is also okay
...
void ecaHandler()
{
  ...
  //use a define here, not just 0x10!
...
{
  /* ARGH ... PLEASE don't wildly mix platform independent and dependent types when accessing other devices!
  / I won't care about an int counter in a for loop, but don't use this outside the lm32!
  / use int16_t and uint16_t, not short etc
  */
  ...
  //use defines or constants, not just a value. Better would be a register map for the mil device!
  //addendum: okay, can see now that Wolfgang's register map in scu_mil.h is a right mess with the 32b-word-addresses...
  //Also, not your fault then with the stdint issue. He even includes stdint and then does not use it ...
  //scu_mil code could do with a makeover

  fcIfb = ifbAddr | (0x13 << 8);
  busStatus = write_mil(pMILPiggy, wData, fcIfb);
  ...
  busStatus = read_mil(pMILPiggy, &rData, fcIfb);
...
int writeToPZU(uint16_t ifbAddr, uint16_t modAddr, uint16_t data)
{
  ...
  /* select module */
  // This looks way better than above! With the define, someone can immediately see what this is about
...
int readFromPZU(uint16_t ifbAddr, uint16_t modAddr, uint16_t *data)
{
  ...
  // better: call constant something like MIL_STAT_OK 
  if (busStatus != OKAY) return busStatus;
...
int initMILEvent(unsigned int *pMILPiggy, uint32_t evtCode, uint32_t virtAcc)
{
  ...
  filter = FI_EV_TO_FIFO | FI_EV_PULS1_S |  FI_EV_PULS2_S;
  set_filter_evt_mil(pMILPiggy,  evtCode, virtAcc, filter); 

  enable_lemo_pulse_evt_mil(pMILPiggy, 1);
...
void MILEvtHandler(uint32_t evtCode, uint32_t virtAcc){
  uint32_t evtDataRec;         /* data of one MIL event                           */
  uint32_t evtCodeRec;         /* "event number"                                  */
  uint32_t virtAccRec;         /* virtual accelerator                             */
  
  while (fifo_notempty_evt_mil(pMILPiggy)) {     /* drain fifo until empty, check */
    pop_fifo_evt_mil(pMILPiggy, (unsigned int*)(&evtDataRec)); /* pop element     */
    evtCodeRec  = evtDataRec & 0x000000ff;
    virtAccRec  = (evtDataRec >> 8) & 0x0f;
    if ((evtCodeRec == evtCode) && (virtAccRec == virtAcc)) {
      /* do action        */
      
      /* debug            */
      if (*pSharedStatus < 500) mprintf("MILEvtHandler: evtCode: 0x%04x, virtAcc: 0x%04x\n", evtCode, virtAcc);
    } /* event received */
  } /* while */
} /* MILEvtHandler */
...
void main(void) {
  ...
  uint32_t uniReady2SisCode;     /* event code for UNI_READY_TO_SIS       */
  uint32_t uniReady2SisAcc;      /* virtual acc for UNI_READY_TO_SIS      */
  ...
  initMILPiggy();
  initMILEvent(pMILPiggy, uniReady2SisCode, uniReady2SisAcc);
  ...
  i=0;
  while (1) {
    MILEvtHandler(uniReady2SisCode, uniReady2SisAcc);
