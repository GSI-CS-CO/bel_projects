...
//
// this is done once per second and some data is printed to screen
// 
...
int main(int argc, const char** argv) {
  ...
  uint64_t          tWR;                  // White Rabbit time             [ns]
  uint64_t          dtTrans;              // length of transaction         [us]
  uint64_t          tStart;               // OS time prior transaction     [us]
  uint64_t          tStop;                // OS time after transaction     [us]
  uint64_t          tTS;                  // assumed OS time of tWR        [us]
  uint64_t          offset;               // UTC offset of system time     [ns]
  uint64_t          nsOffset;             // UTC offset, nanoseconds part
  uint64_t          secsOffset;           // UTC offset, seconds part      
  ...
  while (1) {
    ...
    tStop   = getSysTime();
    dtTrans = tStop - tStart;                // length of transaction
    tTS     = tStart + dtTrans / 2;          // assume symmetry in delay for transaction
    
    tWR   = (uint64_t)data1 << 32;
    tWR  += (uint64_t)data2;
    ...
    ns         = tWR % 1000000000;
    secs       = tWR / 1000000000;
    nsOffset   = offset % 1000000000;
    secsOffset = offset / 1000000000;
    ...
    // print the result 
    printf("current TAI       : %10s.%09lu s\n", time, ns);
    printf("length transaction:         %11u.%09lu s\n", 0, dtTrans * 1000);
    printf("TAI_wr - UTC_os   :         %11lu.%09lu s\n\n", secsOffset, nsOffset);
